...
- ~~**`FilingStatus` only supports `single`/`marriedFilingJointly`**~~ — ✅ **CLOSED in v1.0, struck 2026-08-07.** All four statuses ship: `FilingStatus` is `"single" | "marriedFilingJointly" | "headOfHousehold" | "marriedFilingSeparately"` (`services/tax-engine/src/types.ts:1`), with brackets/standard deduction/Additional Medicare threshold keyed on each, and Head of Household + Married Filing Separately both selectable in `OnboardingScreen` and `EditTaxProfileScreen`. *(This line survived as a false open gap for ~6 weeks after the fix landed, and was still being cited as outstanding. Verified against the code before striking.)*
...
- **Earnings optimizer.** Using the user's own historical data, surface which **days of the week** earn best — personalized, not dependent on surge data you don't have access to. **Shipped in v1.2 at 1.2.6.4, day-of-week only ([D20]).** ⛔ **"Best time-of-day" is NOT buildable and was never costed:** `Entry` records a date and no time, so a claim like "Spark Tuesday *mornings*" has nothing behind it. It needs a data-model change first, and would then read empty for every existing entry. **(time-of-day: v1.3+, premium)**
