...
- [x] **Finalize app identity before submission — DONE.** `apps/mobile/app.json` now has a real display name ("**SetAside**" — ties directly to the dashboard's "Set aside for taxes" feature; **not yet verified for App Store name availability/trademark conflicts**, confirm before final submission), slug (`setaside`), `ios.bundleIdentifier`/`android.package` (`com.gigtaxtracker.app`, shared across both platforms), and initial build numbers. Internal project/package naming (`@gig-tax-tracker/tax-engine`, repo folder names) deliberately left unchanged — only the user-facing app identity changed.
