...
> | **Expect** | ✅ **No compliance step from 2026-09-26 ([D30])** — `ITSAppUsesNonExemptEncryption: false` is in `app.json`, so a build is installable the moment it uploads. ⚠️ **If a build DOES land as "Missing Compliance", the key did not reach Info.plist** — check the prebuild, do not click through the form and move on. |
...
|---|---|---|
| **0** | **⛔ DOES IT UPLOAD — AND THEN, DOES IT INSTALL?** (a) **ITMS-91053** names every required-reason API still undeclared — the one thing about the privacy manifest that cannot be checked off-device (1.2.10.1), and it is decided during Apple's **processing**, not by a green build. **The signal is the build reaching "Ready to Submit".** (b) ✅ **Export compliance is now pre-answered ([D30])** — the build should be installable with no questionnaire. ⚠️ **If it is not, that is a REGRESSION, not a step to click through**: the key failed to reach Info.plist. | A refused or un-installable binary costs the whole cycle before any row below runs. ⚡ **Row 0(b) used to be a manual step on every build; [D30] removed it**, which is why its failure now means something is broken rather than something is owed. |
